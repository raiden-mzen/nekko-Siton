...
const Booking: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (step === totalSteps - 1 && validateStep()) {
      ...
      try {
        ...
        const amount = selectedService ? parseNumber(selectedService.price) : 0;
        ...
        if (formData.paymentProof) {
          ...
          if (uploadErr) console.warn("Upload failed:", uploadErr);
          else {
  ...
  // Render steps exactly as you provided
      ...
      case 2:
        return (
          <>
            ...
            <div className="form-group">
              <label>Service *</label>
              <select name="service" value={formData.service} onChange={handleChange} required disabled={loadingServices}>
                <option value="">{loadingServices ? "Loading services..." : "Select a service"}</option>
                {services.map((s) => <option key={s.id} value={s.title}>{s.title} {s.price ? `- ${s.price}` : ""}</option>)}
              </select>
            ...
            <div className="form-group">
              ...
              <textarea name="message" placeholder="Tell us about your event, preferred time, special requirements..." value={formData.message} onChange={handleChange} rows={4}></textarea>
            </div>
          </>
        );
      case 3:
          ...
          <>
            ...
            <div className="payment-instructions">
              ...
              <div className="gcash-details">
                ...
                <p><strong>Amount:</strong> See pricing on Services page</p>
              </div>
              <ol>
                ...
                {!formData.paymentProof ? (
                  ...
                  <div className="file-preview">
                    ...
                    <div className="file-info">
                      ...
                      <p className="file-size">{(formData.paymentProof.size / 1024).toFixed(2)} KB</p>
                      <button type="button" onClick={removeFile} className="remove-file-btn">Remove</button>
                    </div>
                  </div>
                )}
              </div>
      ...
      case 4:
        return (
          <div className="success-message">
            ...
            <p className="step-subheading">
              Thank you, <strong>{formData.name}</strong>! Your request for <strong>{formData.service}</strong> on <strong>{formData.date}</strong> has been received.
            </p>
            <p>We've sent a confirmation email to <strong>{formData.email}</strong> with the next steps.</p>
            <p className="success-note">We'll verify your payment and get back to you within 24 hours.</p>
          </div>
        );
      default:
  ...
  const completionPercent = Math.min(((step - 1) / (totalSteps - 1)) * 100, 100);

  return (
    <div className="booking-page">
      <div className="booking-container">
        <h1 className="booking-title">Book a Session</h1>
        <p className="booking-subtitle">Fill out the form and we'll get back to you shortly.</p>
          ...
          <div className="booking-form">
            {step < totalSteps && (
              <div className="progress-container">
                ...
                <div className="progress-steps">
                  {["Contact", "Details", "Payment", "Finalize"].map((label, index) => (
                    <div key={index} className={`progress-step-item ${step >= index + 1 ? "active" : ""} ${step > index + 1 ? "completed" : ""}`}>
            ...
            <form className="step-form" onSubmit={handleSubmit}>
              ...
              <div className="form-actions">
                {step > 1 && step < totalSteps && <button type="button" onClick={handlePrev} className="action-btn prev-btn">← Back</button>}
                {step < totalSteps - 1 && <button type="button" onClick={handleNext} className="action-btn next-btn">Next Step →</button>}
                {step === totalSteps - 1 && <button type="submit" className="action-btn submit-btn" disabled={isSubmitting}>{isSubmitting ? "Submitting..." : "Confirm Booking"}</button>}
              </div>
            </form>
          </div>
        </div>
